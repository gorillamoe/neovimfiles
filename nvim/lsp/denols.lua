...
return {
  ...
  root_dir = function(bufnr, on_dir)
    ...
    then
      -- `deno` configuration is closer than or equal to package manager lock,
      -- or `deno` lock is closer than package manager lock. Attach at the project root,
      -- or `deno` lock or `deno` configuration path. At least one of these is always set at this point.
